...
func solveHonestly(n int, s, t string) string {
	convertString := func(n, x int) string {
		res := make([]string, n)
		for i := 0; i < n; i++ {
			if x>>i&1 == 1 {
				res[n-1-i] = "1"
...
func solve(n int, s, t string) string {
	...
	for idx >= 0 && diff != 0 {
		...
		if diff > 0 && s[idx] == '1' {
			ans[idx] = string(s[idx])
			diff -= 2
		} else if diff < 0 && t[idx] == '1' {
			ans[idx] = string(t[idx])
			diff += 2
		}
		idx--
	}
