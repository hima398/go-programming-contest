...
func solve(n, sx, sy, tx, ty int, x, y, r []int) string {
	...
	for i := 0; i < n-1; i++ {
		for j := i + 1; j < n; j++ {
			dist := computeDist2(x[i], y[i], x[j], y[j])
			//ok := (r[i]+r[j])*(r[i]+r[j]) >= dist
			//外接
			ok := dist == (r[i]+r[j])*(r[i]+r[j])
			//交わる
			//大きい方の円に含まれるケース
			if r[i] <= r[j] {
				//ok = ok && r[j]*r[j] < (dist+r[i])*(dist+r[i])
				ok = ok || (r[j]-r[i])*(r[j]-r[i]) <= dist && dist < (r[j]+r[i])*(r[j]+r[i])
			} else {
				// r[i] > r[j]
				//ok = ok && r[i]*r[i] (dist+r[j])*
				ok = ok || (r[i]-r[j])*(r[i]-r[j]) <= dist && dist < (r[i]+r[j])*(r[i]+r[j])
			}
			if ok {
				uf.Unite(i, j)
			}
