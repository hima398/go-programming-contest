...
func solveHonestly(n int, m []int, p, e [][]int) int {
...
func solve(n int, m []int, p, e [][]int) int {
	...
	eMax := make(map[int]int)
	for i := 0; i < n; i++ {
		for j := 0; j < m[i]; j++ {
			eMax[p[i][j]] = Max(eMax[p[i][j]], e[i][j])
		}
	}
	...
	for i := 0; i < n; i++ {
		isDifferent := false
		for j := 0; j < m[i]; j++ {
			isDifferent = isDifferent || e[i][j] == eMax[p[i][j]] && primes[p[i][j]] == 1
		}
		if isDifferent {
			ans++
		} else if !isSame {
