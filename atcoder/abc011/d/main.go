...
func main() {
	...
	n, d := nextInt(), nextInt()
	x, y := nextInt(), nextInt()
	//ans := solveHonestly(n, d, x, y)
	ans := solve(n, d, x, y)
	PrintFloat64(ans)
}

func solve(n, d, x, y int) float64 {
	dpx := make([]map[int]float64, n+1)
	for i := 0; i <= n; i++ {
		dpx[i] = make(map[int]float64)
	}
	dpx[0][0] = 1.0
	for i := 0; i < n; i++ {
		for k, v := range dpx[i] {
	...
	dpy := make([]map[int]float64, n+1)
	for i := 0; i <= n; i++ {
		dpy[i] = make(map[int]float64)
	}
	dpy[0][0] = 1.0
	for i := 0; i < n; i++ {
		for k, v := range dpy[i] {
...
func solveHonestly(n, d, x, y int) float64 {
	...
	for i := 0; i < n; i++ {
		...
		for curX := range m {
			for curY := range m[curX] {
				for k := 0; k < 4; k++ {
					nextX, nextY := curX+d*dx[k], curY+d*dy[k]
					if next[nextX] == nil {
						next[nextX] = make(map[int]int)
					}
					next[nextX][nextY] += m[curX][curY]
				}
			}
		}
		m = next
	...
	for curX := range m {
		for curY := range m[curX] {
			if curX == x && curY == y {
				b += float64(m[curX][curY])
			}
			a += float64(m[curX][curY])
		}
	}
	return b / a
}
