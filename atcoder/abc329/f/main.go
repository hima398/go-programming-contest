...
func solve(n, q int, c, a, b []int) []int {
	boxes := make([]map[int]struct{}, n)
	for i := 0; i < n; i++ {
		boxes[i] = make(map[int]struct{})
		boxes[i][c[i]] = struct{}{}
	}
	var ans []int
	for i := 0; i < q; i++ {
		if boxes[a[i]] == nil && boxes[b[i]] == nil {
			ans = append(ans, 0)
		} else if boxes[a[i]] == nil {
			ans = append(ans, len(boxes[b[i]]))
		} else if boxes[b[i]] == nil {
			boxes[b[i]] = boxes[a[i]]
			ans = append(ans, len(boxes[b[i]]))
			boxes[a[i]] = nil
		} else {
			//fmt.Println(a[i], b[i], it)
			if len(boxes[a[i]]) < len(boxes[b[i]]) {
				for k := range boxes[a[i]] {
					if _, found := boxes[b[i]][k]; !found {
						boxes[b[i]][k] = struct{}{}
					}
				}
			} else {
				for k := range boxes[b[i]] {
					if _, found := boxes[a[i]][k]; !found {
						boxes[a[i]][k] = struct{}{}
					}
				}
				boxes[b[i]] = nil
				boxes[a[i]], boxes[b[i]] = boxes[b[i]], boxes[a[i]]
			}
			ans = append(ans, len(boxes[b[i]]))
			boxes[a[i]] = nil
		}
...
func solve01(n, q int, c, a, b []int) []int {

	boxes := make([]*redblacktree.Tree, n)
	for i := 0; i < n; i++ {
		boxes[i] = redblacktree.NewWithIntComparator()
		boxes[i].Put(c[i], 1)
	}
	var ans []int
	for i := 0; i < q; i++ {
		if boxes[a[i]] == nil && boxes[b[i]] == nil {
			...
			ans = append(ans, boxes[b[i]].Size())
		} else if boxes[b[i]] == nil {
			boxes[b[i]] = boxes[a[i]]
			ans = append(ans, boxes[b[i]].Size())
			boxes[a[i]] = nil
		} else {
			it := boxes[a[i]].Iterator()
			it.First()
			//fmt.Println(a[i], b[i], it)
			for {
				if _, found := boxes[b[i]].Get(it.Key()); !found {
					boxes[b[i]].Put(it.Key(), 1)
				}
				if !it.Next() {
					break
				}
