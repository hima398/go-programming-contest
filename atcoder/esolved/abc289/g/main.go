...
func main() {
	...
	ans := solveCommentary(n, m, b, c)
	//ans := solveHonestly(n, m, b, c)
	PrintHorizonaly(ans)
}

func solveCommentary(n, m int, b, c []int) []int {
	...
	for i := 1; i < n; i++ {
		...
		for {
			cur := ls[len(ls)-1]
			x = cur.b + a - Min(cur.b+a, b+cur.a)/(a-cur.a)
			if cur.x < x {
				break
			}
			ls = ls[:len(ls)-1]
	...
	for _, ci := range c {
		...
		ans = append(ans, ls[idx].a*ci+ls[idx].b)
	}
	return ans
}

//O(MN**2)
func solveHonestly(n, m int, b, c []int) []int {
	var ans []int
	for j := 0; j < m; j++ {
		//k番目の商品の最大の売り上げ
		mx := 0
