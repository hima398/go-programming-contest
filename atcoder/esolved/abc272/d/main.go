...
func computeDist2(i1, j1, i2, j2 int) int {
	d1, d2 := i1-i2, j1-j2
	return d1*d1 + d2*d2
}
func solve(n, m int) ([][]int, error) {
	...
	for len(q) > 0 {
		...
		q = q[1:]
		for k, v := range x2 {
			if _, found := x2[m-k]; !found {
				continue
			}
			bi, bj := v, x2[m-k]
			//fmt.Println(cur.i, cur.j, bi, bj)
			for l := 0; l < 4; l++ {
				ni, nj := cur.i+di[l]*bi, cur.j+dj[l]*bj
