...
func main() {
	...
	ans := solve(n, k, a, q, l, r)
	PrintVertically(ans)
}

func solve(n, k int, a []int, q int, l, r []int) []string {
	s := make([][]int, k)
	for i := 0; i < k; i++ {
		s[i] = make([]int, n+1)
	}
...
func firstsolve(n, k int, a []int, q int, l, r []int) []string {
	sl := make([]int, n+1)
	//sr := make([]int, n+1)
	for i := 1; i <= n; i++ {
		sl[i] = sl[i-1] + a[i-1]
	...
	for i := 0; i < q; i++ {
		//左から順に計算
		ll := sl[r[i]] - sl[l[i]-1]
		//rr := sr[n-l[i]+1] - sr[n-r[i]]
		if ll%k == 0 && (r[i]-l[i]+1)%k == 0 {
			ans = append(ans, "Yes")
		} else {
			ans = append(ans, "No")
		}
