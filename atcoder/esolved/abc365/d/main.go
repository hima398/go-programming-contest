...
func solve(n int, s string) int {
	hand := map[byte]int{'R': 0, 'P': 1, 'S': 2}
	janken := func(t, a int) int {
		switch t {
		case 0:
			switch a {
			case 0:
	...
	//0:R, 1:P, 2:S
	dp := make([][3]int, n)
	dp[0][0] = janken(0, hand[s[0]])
	dp[0][1] = janken(1, hand[s[0]])
	dp[0][2] = janken(2, hand[s[0]])
	for i := 1; i < n; i++ {
		//i-1番目に何を出したか
		for j := 0; j < 3; j++ {
			if dp[i-1][j] < 0 {
			...
			for k := 0; k < 3; k++ {
				...
				dp[i][k] = Max(dp[i][k], dp[i-1][j]+janken(k, hand[s[i]]))
			}
		}
	}
