...
func solve(n, m, a, b, c, d, e, f int, x, y []int) int {
	const p = 998244353
	obstacles := make(map[int]map[int]struct{})
	for i := 0; i < m; i++ {
		if obstacles[x[i]] == nil {
			obstacles[x[i]] = make(map[int]struct{})
		}
		obstacles[x[i]][y[i]] = struct{}{}
	}
	dp := make([][][]int, n+1)
	for i := 0; i <= n; i++ {
		dp[i] = make([][]int, n+1)
	...
	for i := 0; i < n; i++ {
		for j := 0; j <= i; j++ {
			for k := 0; k <= i-j; k++ {
				l := i - j - k
				nx, ny := j*a+k*c+l*e, j*b+k*d+l*f
				if _, found := obstacles[nx+a][ny+b]; !found {
					dp[i+1][j+1][k] += dp[i][j][k]
					dp[i+1][j+1][k] %= p
				}
				if _, found := obstacles[nx+c][ny+d]; !found {
					dp[i+1][j][k+1] += dp[i][j][k]
					dp[i+1][j][k+1] %= p
				}
				if _, found := obstacles[nx+e][ny+f]; !found {
					dp[i+1][j][k] += dp[i][j][k]
					dp[i+1][j][k] %= p
				}
			}
