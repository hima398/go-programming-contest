...
func solve(a, b int) float64 {
	//ba, bb := big.NewFloat(float64(a)), big.NewFloat(float64(b))
	f1 := func(x float64) float64 {
		return (x)*float64(b) + float64(a)/math.Sqrt(x+1)
	}
	ok, ng := 0, int(1e18)
	for ng-ok > 1 {
		mid := 
	}
	x := sort.Search(int(1e18), func(i int) bool {
		return f1(float64(i)) < f1(float64(i+1))
	})
	fmt.Println(x)
	ans := f1(float64(x - 1))
	...
	/*
		f := func(x float64) float64 {
			bx := big.NewFloat(x)
			bx = bx.Sub(bx, big.NewFloat(1.0))
			bb = bb.Mul(bb, bx)
			sx := big.NewFloat(x)
			sx = sx.Sqrt(sx)

			return (x-1)*float64(b) + float64(a)/math.Sqrt(x)
		}
	*/
	return ans
}
