...
func solve(n int, d [][]int) int {
	s := 1<<n - 1
	//点を選んだ集合tと最後に選んだ点がiのときの重みの総和の最大
	dp := make([][]int, s+1)
	for t := range dp {
		dp[t] = make([]int, n)
	}
	...
	sort.Slice(pat, func(i, j int) bool {
		if bits.OnesCount(uint(pat[i])) == bits.OnesCount(uint(pat[j])) {
			return pat[i] < pat[j]
		}
		return bits.OnesCount(uint(pat[i])) < bits.OnesCount(uint(pat[j]))
	})
	for _, t := range pat {
		//最後に選ばれた点がi、次にjを選択する
		for i := 0; i < n; i++ {
			...
			for j := 0; j < n; j++ {
				...
				dp[next][j] = Max(dp[next][j], dp[t][i]+cost)
			}
		}
	}
