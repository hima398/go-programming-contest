...
func solve(n, d int, x, y []int) []string {
	type pos struct {
		i, x, y int
	}
	visited := make([]bool, n)
	var q []pos
	q = append(q, pos{0, x[0], y[0]})
	visited[0] = true
	for len(q) > 0 {
		cur := q[0]
		q = q[1:]
		for next := 0; next < n; next++ {
			...
			d2 := computeDist2(cur.x, cur.y, x[next], y[next])
			if d2 <= d*d {
				q = append(q, pos{next, x[next], y[next]})
				visited[next] = true
			}
		}
	}
...
func solve01(n, d int, x, y []int) []string {
	ans := make([]string, n)
	for i := range ans {
		ans[i] = "No"
	}
	...
	for i := 0; i < n; i++ {
		...
		for j := 0; j < n; j++ {
			...
			d2 := computeDist2(x[i], y[i], x[j], y[j])
			if d2 <= d*d {
				ans[j] = "Yes"
			}
		}
