...
func firstsolve(n, x, y int, a []int) string {
	dx := []int{1, 0, -1, 0}
	dy := []int{0, 1, 0, -1}
	k := 0
	curX, curY := 0, 0
...
func secondsolve(n, x, y int, a []int) string {
	dp := make(map[int]map[int]struct{})
	//dx := []int{1, 0, -1, 0}
	//dy := []int{0, 1, 0, -1}
	dp[a[0]] = make(map[int]struct{})
	dp[a[0]][0] = struct{}{}
	for i := 1; i < n; i++ {
		...
		for x, m := range dp {
			for y := range m {
				if i%2 == 0 {
					//k=0, 2
					if next[x+a[i]] == nil {
						next[x+a[i]] = make(map[int]struct{})
					}
					next[x+a[i]][y] = struct{}{}
					if next[x-a[i]] == nil {
						next[x-a[i]] = make(map[int]struct{})
...
func solve(n, x, y int, a []int) string {
	...
	dpy[0] = struct{}{}
	for i := 1; i < n; i++ {
		var nextX map[int]struct{}
		var nextY map[int]struct{}
		if i%2 == 0 {
			//k=0, 2
			// x方向の更新
			nextX = make(map[int]struct{})
			for key := range dpx {
				nextX[key+a[i]] = struct{}{}
				nextX[key-a[i]] = struct{}{}
			}
			dpx = nextX
		} else {
			//k=1, 3
			// y方向の更新
			nextY = make(map[int]struct{})
			for key := range dpy {
				nextY[key+a[i]] = struct{}{}
				nextY[key-a[i]] = struct{}{}
			}
			dpy = nextY
		}
