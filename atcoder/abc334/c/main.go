...
func solve(n, k int, a []int) int {
	if k%2 == 0 {
		...
		//fmt.Println("0を残す:", ans)
		var s int
		for i := 1; i < k-1; i += 2 {
			s += a[i+1] - a[i]
		}
		ans = Min(ans, s)
		//fmt.Println("4を残す", ans)
		...
		for j := 1; j < k-1; j++ {
			if j%2 == 1 {
				ans = Min(ans, s-(a[j+1]-a[j])-(a[j]-a[j-1])+(a[j+1]-a[j-1]))
			} else {
				ans = Min(ans, s-(a[j+1]-a[j])-(a[j]-a[j-1]))
			}
		}
		return ans
	}
...
func solve00(n, k int, a []int) int {
	type pair struct {
		d, i, j int
	}
	var ps []pair
	for i := 0; i < k-1; i++ {
		ps = append(ps, pair{a[i+1] - a[i], a[i], a[i+1]})
	}
	...
	used := make([]bool, 2*int(1e5)+1)
	var ans int
	for len(ps) > 0 {
		cur := ps[0]
		ps = ps[1:]
		if used[cur.i] || used[cur.j] {
			continue
		}
		used[cur.i] = true
		used[cur.j] = true
