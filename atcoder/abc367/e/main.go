...
func buildDoublingTable(N int, maxStep int, adj []int) [][]int {
	dp := make([][]int, maxStep)
	for i := 0; i < maxStep; i++ {
		dp[i] = make([]int, N)
	}
...
func solve(n, k int, x, a []int) []int {
	maxStep := 60

	dp := buildDoublingTable(n, maxStep, x)
	...
	for i := 0; i < n; i++ {
		...
		finalPositions[i] = cur
	}
	fmt.Println(finalPositions)

	ans := make([]int, n)
	for i := 0; i < n; i++ {
		ans[i] = a[finalPositions[i]]
	}
