...
func solve(n int, a []int) []int {
	...
	visited := make([]int, n)
	var dfs func(cur, root int) ([]int, int, bool, bool)
	dfs = func(cur, root int) ([]int, int, bool, bool) {
		visited[cur]++
		if visited[cur] == 2 {
			return []int{}, cur, true, false
		}
		for _, next := range e[cur] {
			res, leaf, found, finished := dfs(next, root)
			if found {
				if !finished {
				...
				return res, leaf, found, finished
			}
		}
