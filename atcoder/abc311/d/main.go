...
func solve(n, m int, s []string) int {
	...
	for len(q0) > 0 || len(q1) > 0 {
		...
		//if s[cur.i+di[cur.dir]][cur.j+dj[cur.dir]] == '#' {
		//	//岩にぶつかって止まる
		//	cur.dir = -1
		//}
		if cur.dir < 0 { //止まっている
			for k := 0; k < 4; k++ {
				...
				if s[ni][nj] == '#' || visited[ni][nj][k] {
					continue
				}
				q0 = append(q0, cell{ni, nj, k})
				visited[ni][nj][k] = true
			}
		} else { //滑っている
			ni, nj := cur.i+di[cur.dir], cur.j+dj[cur.dir]
			if s[ni][nj] == '.' {
				q0 = append(q0, cell{ni, nj, cur.dir})
				visited[ni][nj][cur.dir] = true
			} else {
				q1 = append(q1, cell{cur.i, cur.j, -1})
			}
		}
	}
	var ans int
