...
func solve(n, m int, s []string) int {
	const p = 998244353
	//マス(i, j)がk(k==0のとき黒、k==1のとき白)の時の美しいグリッドの数
	dp := make([][]map[byte]int, n)
	for i := range dp {
		dp[i] = make([]map[byte]int, m)
		for j := range dp[i] {
			dp[i][j] = make(map[byte]int)
		}
	}
	dp[n-1][m-1]['#'] = 1
	if s[n-1][m-1] == '.' {
	...
	for j := m - 2; j >= 0; j-- {
		dp[n-1][j]['#'] = dp[n-1][j+1][s[n-1][j+1]]
		if s[n-1][j] == '.' {
			dp[n-1][j]['.'] = (2 * dp[n-1][j]['#']) % p
		}
	}
	for i := n - 2; i >= 0; i-- {
		dp[i][m-1]['#'] = dp[i+1][m-1]['#']
		if s[i][m-1] == '.' {
			dp[i][m-1]['.'] = (2 * dp[i][m-1]['#']) % p
		}
	}
	for i := n - 2; i >= 0; i-- {
		for j := m - 2; j >= 0; j-- {
