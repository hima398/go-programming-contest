...
func main() {
	...
	q := nextInt()
	t, day, height := make([]int, q), make([]int, q), make([]int, q)
	for i := 0; i < q; i++ {
		t[i] = nextInt()
		switch t[i] {
		case 2:
	...
	ans := solve(q, t, day, height)
	for _, v := range ans {
		Print(v)
	}
}

func solve(q int, t, day, height []int) []int {
	var ans []int
	var totaldays int
	queue := priorityqueue.New[int](comparator.IntComparator)
	for i := 0; i < q; i++ {
		switch t[i] {
		case 1:
			queue.Push(totaldays)
		case 2:
			totaldays += day[i]
		case 3:
			var s int
