...
func solve(n, m int, x, a []int) (int, error) {
	var s int
	for _, ai := range a {
		s += ai
	}
	...
	var stones []stone
	for i := 0; i < m; i++ {
		stones = append(stones, stone{x[i], a[i]})
	}
	...
	if stones[m-1].x != n {
		stones = append(stones, stone{n, 0})
		m++
	}

	var ans int
	for i := 0; i < m-1; i++ {
		l, r := stones[i].x, stones[i+1].x
		need := r - l - 1
		/*
			if a[i+1] > 0 {
				need--
