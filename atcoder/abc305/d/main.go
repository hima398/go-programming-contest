...
func solve(n int, a []int, q int, l, r []int) []int {
	...
	var ps []period
	for i := 0; i < n; i++ {
		ps = append(ps, period{i, a[i], a[i+1], i % 2})
	}
	//ps = append(ps, period{n, a[n-1], int(1e9) + 1, 0})
	//fmt.Println(ps)
	s := make([]int, n+1)
	for i := 1; i < n; i++ {
		s[i] = s[i-1] + (ps[i-1].t-ps[i-1].s)*ps[i-1].w
	}
	s[n] = s[n-1]
	//fmt.Println(s)
	var ans []int
	for i := 0; i < q; i++ {
		li := sort.Search(len(ps), func(j int) bool {
			return l[i] < ps[j].t
		})
		ri := sort.Search(len(ps), func(j int) bool {
			return r[i] < ps[j].t
		})
		//fmt.Println(li, ri)
		v := s[ri] - s[li]
		//fmt.Println(v)
		v += (Abs(r[i] - ps[ri].s)) * ps[ri].w
		v -= (Abs(l[i] - ps[li].s)) * ps[li].w
		ans = append(ans, v)
	}
	return ans
}
