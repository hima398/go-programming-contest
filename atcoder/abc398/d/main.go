...
func solve(n, r, c int, s string) string {
	dir := map[rune][2]int{
		'N': [2]int{1, 0},  //下
		'E': [2]int{0, -1}, //左
		'W': [2]int{0, 1},  //右
	...
	field := make(map[int]map[int]struct{})
	var ans []string
	fire := [2]int{0, 0}
	field[0] = make(map[int]struct{})
	field[0][0] = struct{}{}
	for _, si := range s {
		fire[0], fire[1] = fire[0]+dir[si][0], fire[1]+dir[si][1]
		if field[fire[0]] == nil {
			field[fire[0]] = make(map[int]struct{})
		}
		field[fire[0]][fire[1]] = struct{}{}
		r, c = r+dir[si][0], c+dir[si][1]
		if _, found := field[r][c]; found {
			ans = append(ans, "1")
		} else {
			ans = append(ans, "0")
		}
