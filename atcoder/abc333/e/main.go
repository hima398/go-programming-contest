...
func solve(n int, t, x []int) (int, []int, error) {
	m := make(map[int][]int)
	a := make([]int, n)
	for i := 0; i < n; i++ {
		switch t[i] {
		case 1:
			m[x[i]] = append(m[x[i]], i)
		case 2:
			if len(m[x[i]]) == 0 {
				return -1, nil, errors.New("Impossible")
			}
			idx := m[x[i]][len(m[x[i]])-1]
			m[x[i]] = m[x[i]][:len(m[x[i]])-1]
			a[idx] = 1
			a[i] = 2
		}
		//fmt.Println(m)
