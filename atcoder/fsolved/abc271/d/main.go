...
func solve(n, s int, a, b []int) (string, error) {
	//i枚目まで見て、総和がj
	dp := make([][]bool, n+1)
	for i := 0; i <= n; i++ {
		dp[i] = make([]bool, int(1e4)+1)
	}
	dp[0][0] = true
	for i := 1; i <= n; i++ {
		for j := 0; j <= s; j++ {
	...
	for i := n; i >= 1; i-- {
		//表にできる
		next := cur - a[i-1]
		if next >= 0 && dp[i-1][cur-a[i-1]] {
			buf = append(buf, "H")
			cur -= a[i-1]
			continue
		}
		next = cur - b[i-1]
		if next >= 0 && dp[i-1][cur-b[i-1]] {
			buf = append(buf, "T")
			cur -= b[i-1]
		}
		//最低限構築可能なのは前段でチェックしているのでelseがなくても良いはず
	}
	//fmt.Println(buf)
