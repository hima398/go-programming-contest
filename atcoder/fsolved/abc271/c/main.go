...
func firstsolve(n int, a []int) int {
	a = append(a, 0)
	sort.Ints(a)
	need := make([]int, n)
	for i := 0; i < n; i++ {
	...
	for i := 0; i < n; i++ {
		if force {
			sell = append(sell, a[i+1])
		} else {
			canBy := (n - soled - i - 1) / 2
			fmt.Println(i, a[i+1], need[i], canBy)
			if need[i] <= canBy {
				use = append(use, a[i+1])
				soled += 2 * need[i]
			} else {
				sell = append(sell, a[i+1])
				force = true
	...
	ans := len(use) + len(sell)/2
	return ans
}

func solveBinarySearch(n int, a []int) int {
	sort.Ints(a)
	ok, ng := 0, 3*int(1e5)+1
