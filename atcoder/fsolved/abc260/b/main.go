...
func main() {
	...
	n, x, y, z := nextInt(), nextInt(), nextInt(), nextInt()
	a := nextIntSlice(n)
	b := nextIntSlice(n)
	var ss []students
	for i := 0; i < n; i++ {
		ss = append(ss, students{i + 1, a[i], b[i], a[i] + b[i]})
	}
	var ans []int
	sort.Slice(ss, func(i, j int) bool {
		if ss[i].math == ss[j].math {
			return ss[i].id < ss[j].id
		}
		return ss[i].math > ss[j].math
	})
	for i := 0; i < x; i++ {
		ans = append(ans, ss[0].id)
		ss = ss[1:]
	}
	sort.Slice(ss, func(i, j int) bool {
		if ss[i].en == ss[j].en {
