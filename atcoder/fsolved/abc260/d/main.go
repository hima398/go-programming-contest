...
func solve(n, k int, p []int) []int {
	...
	uf := NewUnionFind(n)
	for i := 0; i < n; i++ {
		node, found := rbTree.Ceiling(p[i])

		if found {
			uf.Unite(p[i], node.Key.(int))
			if node.Value.(int) == k-1 {
				// 答えを記録
				memo[uf.Find(node.Key.(int))] = i + 1
				rbTree.Remove(node.Key)
			} else {
				rbTree.Put(p[i], node.Value.(int)+1)
				rbTree.Remove(node.Key)
			}
		} else {
			if k == 1 {
