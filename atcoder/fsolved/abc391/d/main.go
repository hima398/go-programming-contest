...
func solve(n, w int, x, y []int, q int, t, a []int) []bool {
	type block struct {
		i, y int
	}
	bs := make([][]block, w)
	for i := 0; i < n; i++ {
		bs[x[i]] = append(bs[x[i]], block{i, y[i]})
	}
	for i := 0; i < w; i++ {
		sort.Slice(bs[i], func(j, k int) bool {
			return bs[i][j].y < bs[i][k].y
		})
	}
	...
	for ok {
		...
		for i := 0; i < w; i++ {
			...
			max = Max(max, bs[i][0].y)
		}
		if !ok {
			break
		}
		for i := 0; i < w; i++ {
			removed[bs[i][0].i] = max + 1
			bs[i] = bs[i][1:]
		}
		//time = max + 1
	}
	...
	var ans []bool
	for i := 0; i < q; i++ {
		ans = append(ans, removed[a[i]] > t[i])
	}
	return ans
}
