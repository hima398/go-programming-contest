...
func solve(n int, u, v []int, q int, x, y []int) (ans []string) {
	type Edge struct {
		i, s, t int
	}
	for i := 0; i < n; i++ {
	...
	for i := 0; i < n; i++ {
		if e[u[i]] == nil {
			e[u[i]] = make(map[int]int)
		}
		e[u[i]][v[i]] = i
		d[u[i]]++
		if e[v[i]] == nil {
			e[v[i]] = make(map[int]int)
		}
		e[v[i]][u[i]] = i
		d[v[i]]++
	}
	...
	for len(queue) > 0 {
		...
		queue = queue[1:]
		for t, ei := range e[s] {
			needEdges = append(needEdges, Edge{ei, s, t})
			delete(e[t], s)
			if len(e[t]) == 1 {
				needEdges = append(needEdges, Edge{ei, s, t})
	...
	//fmt.Println(needEdges)
	uf := NewUnionFind(n)
	for _, edge := range needEdges {
