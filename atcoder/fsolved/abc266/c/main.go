...
func firstSolve(x, y []int) string {
	var sa []float64
	for i := 0; i < 4; i++ {
		i1, i2 := (i+1)%4, (i-1+4)%4
		a := computeAngle(x[i], y[i], x[i1], y[i1], x[i2], y[i2])
		//fmt.Println(i, i1, i2, a, rToD(a))
		sa = append(sa, a)
		//if a > math.Pi {
		//	return "No"
		//}
...
func solve(x, y []int) string {
	var c []complex128
	for i := 0; i < 4; i++ {
		c = append(c, complex(float64(x[i]), float64(y[i])))
	}
	check := func(x0, x1, x2 complex128) bool {
		a, b := x1-x0, x2-x0
		outerProduct := real(a)*imag(b) - imag(a)*real(b)
		//fmt.Println(outerProduct)
		return outerProduct > 0
	}
	for i := 0; i < 4; i++ {
		if !check(c[i], c[(i+1)%4], c[(i+3)%4]) {
			return "No"
		}
	}
	return "Yes"
