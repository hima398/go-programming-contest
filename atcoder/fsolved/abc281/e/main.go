...
func solve(n, m, k int, a []int) []int {
	rb1, rb2 := redblacktree.NewWithIntComparator(), redblacktree.NewWithIntComparator()

	add := func(rb *redblacktree.Tree, v int) {
		node, found := rb.Get(v)
		if found {
			rb.Put(v, node.(int)+1)
		} else {
	...
	remove := func(rb *redblacktree.Tree, v int) {
		if rb.Size() == 0 {
			return
		}
		node, found := rb.Get(v)
	...
	for r := m; r < n; r++ {
		...
		//fmt.Printf("rb2: %v\n", rb2)
		//除去する
		if a[l] <= rb1.Right().Key.(int) {
			remove(rb1, a[l])
			s -= a[l]
			if rb2.Size() > 0 {
				left := rb2.Left().Key.(int)
				add(rb1, left)
				s += left
				remove(rb2, left)
			}
		...
		if rb1.Size() == 0 {
			add(rb1, a[r])
			s += a[r]
		} else if a[r] <= rb1.Right().Key.(int) {
			add(rb1, a[r])
			s += a[r]
			right := rb1.Right().Key.(int)
			remove(rb1, right)
			s -= right
			add(rb2, right)
		} else {
