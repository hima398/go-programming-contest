...
func main() {
	...
	ans := solveHonestly(n, m, k, u, v, c)

	PrintInt(ans)
}

func solveHonestly(n, m, k int, u, v, c []int) int {
	const p = 998244353

	//i回目の行動で、頂点jにレベルlで到達する確率
	dp := make([][][]int, k+1)
	for i := 0; i <= k; i++ {
		dp[i] = make([][]int, n)
		for j := 0; j < n; j++ {
	...
	inv := make([]int, n)
	for i := 0; i < n; i++ {
		//グラフは連結なので、len(e[i])==0は条件を書かなくて良いはず
		//if len(e[i]) == 0 {
		//	continue
		//}
		inv[i] = Inv(len(e[i]), p)
