...
func solve1(n int, a []int) int {
	...
	//dp[0][0] = 1 << 60
	//dp[0][1] = a[n-1]
	// 1番めの動物には少なくともa[0]をあげる
	dp1[1][0] = 1 << 60
	dp1[1][1] = a[0]
	for i := 2; i <= n; i++ {
		dp1[i][0] = dp1[i-1][1]
		dp1[i][1] = Min(dp1[i-1][0]+a[i-1], dp1[i-1][1])
	}
	var ans []int
	ans = append(ans, dp1[n][0])
	ans = append(ans, dp1[n][1])

	// 1番めの動物には少なくともa[n-1]をあげる
	dp2 := make([][]int, n+2)
	for i := 0; i <= n+1; i++ {
		dp2[i] = make([]int, 2)
	}
	...
	for i := 1; i < n; i++ {
		dp2[i][0] = dp2[i-1][1]
		dp2[i][1] = Min(dp2[i-1][0]+a[i-1], dp2[i-1][1])
	}
	ans = append(ans, dp2[n-1][0])
	ans = append(ans, dp2[n-1][1])
...
func solve(n int, a []int) int {
	...
	//動物1には少なくともエサa[n-1]を与える
	dp2 := make([][]int, n+2)
	for i := 0; i <= n+1; i++ {
		dp2[i] = make([]int, 2)
	}
	a = append([]int{a[n-1]}, a...)
	dp2[1][0] = 1 << 60
	dp2[1][1] = a[0]
	...
	ans := []int{dp1[n][0], dp1[n][1], dp2[n][0], dp2[n][1]}
	sort.Ints(ans)
	return ans[0]
}
