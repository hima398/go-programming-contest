...
func solve(h, w, k int, s []string) int {
	di := []int{0, -1, 0, 1}
	dj := []int{-1, 0, 1, 0}
	var ans int

	var dfs func(i, j, depth int, visited [][]bool)
	dfs = func(i, j, depth int, visited [][]bool) {
		visited[i][j] = true
		if depth == k {
			//fmt.Println(i, j)
			ans++
