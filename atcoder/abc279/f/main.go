...
func solve(n, q int, t, x, y []int) []int {
	var ans []int
	//ボールのまとまりを管理
	balles := NewUnionFind(n + q + 2)
	//ボールがインデックス、箱の番号が値
	ballToBox := make([]int, n+q+2)
	for i := 0; i < n+q+2; i++ {
		ballToBox[i] = 0
	}
	for i := 1; i <= n; i++ {
	...
	//箱の番号がインデックス、ボールの番号が値
	boxToBall := make([]int, n+1)
	for i := 1; i <= n; i++ {
		boxToBall[i] = i
	}
	k := n
	for i, ti := range t {
		switch ti {
		case 1:
			rootX, rootY := balles.Find(x[i]), balles.Find(y[i])
			balles.Unite(rootX, rootY)
			newRoot := balles.Find(x[i])
			//マージしたボールを箱と関連付けする
			ballToBox[newRoot] = x[i]
			if newRoot == rootX {
				ballToBox[rootY] = 0
			} else {
				ballToBox[rootX] = 0
			}
			boxToBall[x[i]] = newRoot
			boxToBall[y[i]] = 0
		case 2:
			k++
			newBall := k
			ballToBox[newBall] = x[i]
			boxToBall[x[i]] = newBall
		case 3:
			rootX := balles.Find(x[i])
			fmt.Println("idx = ", rootX)
			fmt.Println(ballToBox)
			PrintUnionFind(balles)
			box := ballToBox[rootX]
			ans = append(ans, box)
		}
	}
