...
func main() {
	...
	a, b, c, d := nextInt()-1, nextInt()-1, nextInt()-1, nextInt()-1
	...
	q := deque.New[[2]int]()
	//q.PushFront([2]int{a, b})
	//dist[a][b] = 0
	q.PushFront([2]int{c, d})
	dist[c][d] = 0
	//fmt.Println(dist[0])
	for !q.Empty() {
		cur := q.PopFront()
		...
		for _, dir := range dirs {
			...
			if s[next[0]][next[1]] == '.' {
				q.PushFront(next)
				dist[next[0]][next[1]] = dist[cur[0]][cur[1]]
			} else if s[next[0]][next[1]] == '#' {
				q.PushBack(next)
				dist[next[0]][next[1]] = dist[cur[0]][cur[1]] + 1

				n2 := [2]int{next[0] + dir[0], next[1] + dir[1]}
				if n2[0] < 0 || n2[0] >= h || n2[1] < 0 || n2[1] >= w {
					continue
				}
				if dist[n2[0]][n2[1]] < INF {
					continue
				}
				if s[n2[0]][n2[1]] == '.' {
					continue
				...
				q.PushBack(n2)
				dist[n2[0]][n2[1]] = dist[cur[0]][cur[1]] + 1
			}
		}
	}
	/*
