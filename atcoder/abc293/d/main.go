...
func main() {
	...
	x, y := solve(n, m, a, b, c, d)
	fmt.Println(x, y)
}

func solve(n, m int, a []int, b []string, c []int, d []string) (x, y int) {
	uf := NewUnionFind(n * 2)
	for i := 0; i < n; i++ {
		uf.Unite(i, n+i)
	}
	isCircle := make([]bool, 2*n)
	for i := 0; i < m; i++ {
		u, v := a[i], c[i]
		if b[i] == "B" {
			u += n
		...
		if uf.ExistSameUnion(u, v) {
			x++
			isCircle[uf.Find(u)] = true
		} else {
			uf.Unite(u, v)
		}
	}
	for i := 0; i < 2*n; i++ {
		if uf.Find(i) == i && !isCircle[uf.Find(i)] {
			y++
		}
	}
	return x, y
