...
func solve(n, m int, a []int) int {
	...
	uf := NewUnionFind(m, a)
	for _, ai := range a {
		if _, found := ma[(ai+1)%m]; found {
			if uf.ExistSameUnion(ai%m, (ai+1)%m) {
				continue
			}
			pa, pa1 := uf.Find(ai%m), uf.Find((ai+1)%m)
			uf.Unite(ai%m, (ai+1)%m)
			par := uf.Find(ai % m)
			if par == pa {
				ms[pa] += ms[pa1]
			} else {
	...
	ans := s
	for _, ai := range a {
		ans = Min(ans, s-ms[uf.Find(ai%m)])
	}
	return ans
}

type UnionFind struct {
	parent map[int]int //[]int // parentent numbers
	rank   map[int]int //[]int // height of tree
...
func NewUnionFind(m int, x []int) *UnionFind {
	//if n <= 0 {
	//	return nil
	//}
	u := new(UnionFind)
	// for accessing index without minus 1
	u.parent = make(map[int]int) //make([]int, n+1)
	u.rank = make(map[int]int)   //make([]int, n+1)
	u.size = make(map[int]int)   //make([]int, n+1)
	for _, xi := range x {
		u.parent[xi%m] = xi % m
		u.rank[xi%m] = 0
		u.size[xi%m] = 1
