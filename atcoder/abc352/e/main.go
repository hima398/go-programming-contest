...
func solve(n, m int, k, c []int, a [][]int) (int, error) {
	//fmt.Println(n, m)
	//fmt.Println(k, c)
	//for _, v := range a {
	//	fmt.Println(v)
	...
	var qs []query
	for i := 0; i < m; i++ {
		qs = append(qs, query{k[i], c[i], a[i]})
		for j := range qs[i].a {
			qs[i].a[j]--
		}
	}
