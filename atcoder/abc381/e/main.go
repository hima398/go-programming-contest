...
func solve(n, q int, s string, l, r []int) []int {
	idxes := make([][]int, 3)
	for i, si := range s {
		switch si {
		case '/':
			idxes[0] = append(idxes[0], i)
		case '1':
			idxes[1] = append(idxes[1], i)
		case '2':
			idxes[2] = append(idxes[2], i)
		}
	}

	floor := func(s []int, x int) int {
		return sort.Search(len(s), func(i int) bool {
			return x >= s[i]
		})
	}

	f := func(l, r, x int) bool {
		if x == 0 {
			j := floor(idxes[0], l)
			if j >= len(idxes[0]) {
				return false
