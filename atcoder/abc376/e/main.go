...
func main() {
	...
	var ans []int
	for i := 0; i < t; i++ {
		n, k = append(n, nextInt()), append(k, nextInt())
		a = append(a, nextIntSlice(n[i]))
		b = append(b, nextIntSlice(n[i]))
		ans = append(ans, solve(n[i], k[i], a[i], b[i]))
	}
...
func solve(n, k int, a, b []int) int {
	...
	var ns []node
	for i := 0; i < n; i++ {
		ns = append(ns, node{a[i], b[i]})
	}
	sort.Slice(ns, func(i, j int) bool {
		if ns[i].a == ns[j].a {
			return ns[i].b < ns[i].b
		}
		return ns[i].a < ns[j].a
	})
	var maxA, sumB int
	q := priorityqueue.New[int](func(a, b int) int {
		if a > b {
			return -1
		}
		if a == b {
