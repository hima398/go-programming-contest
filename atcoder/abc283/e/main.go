...
func eval(h, w int, a [][]int) (int, error) {
	...
	for i := 1; i < h; i++ {
		...
		for j := 0; j < w; j++ {
			ok[i][j] = ok[i][j] || cur[j] == a[i-1][j]
		}
	}
	notError := true
	for j := 0; j < w; j++ {
		notError = notError && ok[h-1][j]
	}
	if !notError {
