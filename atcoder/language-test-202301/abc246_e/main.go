...
func main() {
	...
	n := nextInt()
	ax, ay := nextInt()-1, nextInt()-1
	bx, by := nextInt()-1, nextInt()-1
	var s []string
	for i := 0; i < n; i++ {
		s = append(s, nextString())
	}
	ans := solve(n, ax, ay, bx, by, s)
	Print(ans)
}

func solve(n, sx, sy, tx, ty int, s []string) int {
	type node struct {
		i, j, d, dir int
	}
	dist := make([][]int, n)
	...
	for !q.Empty() {
		...
		for k := 0; k < 4; k++ {
			...
			if dist[ni][nj] < math.MaxInt {
				continue
			}
			if cur.dir == k {
				q.PushFront(node{ni, nj, cur.d, k})
				dist[ni][nj] = cur.d
			} else {
				q.PushBack(node{ni, nj, cur.d + 1, k})
				dist[ni][nj] = cur.d + 1
			}
		}
	}
