...
func solveCommentary(n int, a []int, s string) int {

}

func solve(n int, a []int, s string) int {
	computeMex2 := func(mask int) (int, error) {
		switch mask {
		case 0:
	...
	for i := 0; i < n; i++ {
		...
		if s[i] == 'X' {
			...
			for j := 1; j <= 7; j++ {
				...
				//mex, err := computeMex(a[i], j, k)
				mex, err := computeMex2(j | (1 << a[i]))
				//fmt.Println(i, j, w, mex)
				if err != nil {
					//意図的に失敗させる
					panic(err)
				}
