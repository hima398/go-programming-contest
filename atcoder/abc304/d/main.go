...
func main() {
	...
	lb := nextInt()
	b := nextIntSlice(lb)
	min, max := solve01(w, h, n, p, q, la, a, lb, b)
	fmt.Println(min, max)
}

//func solve(w, h, n int, p, q []int, la int, a []int, lb int, b []int) (int, int) {
//
//}

func solve01(w, h, n int, p, q []int, la int, a []int, lb int, b []int) (int, int) {
	x := NewCompress()
	x.Init(a)
	y := NewCompress()
	y.Init(b)
	field := make([]map[int]int, la+1)
	for i := 0; i <= la; i++ {
	...
	for i := 0; i < n; i++ {
		idxX := x.GetIndex(p[i])
		idxY := y.GetIndex(q[i])
		//fmt.Println(idxX, idxY)
		field[idxX][idxY]++
	}
	//fmt.Println(field)
	//(A+1)*(B+1)このケーキのうち、苺が載っているケーキの数
	var nf int
	min, max := n+1, 0
	for i := 0; i <= la; i++ {
		nf += len(field[i])
