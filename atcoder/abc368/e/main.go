...
func solve(n, m, x int, a, b, s, t []int) []int {
	const INF = 1 << 60
	e := make([][]edge, n)
	var start edge
	minS := INF
	for i := 0; i < m; i++ {
		e[a[i]] = append(e[a[i]], edge{i, a[i], b[i], s[i], t[i]})
		if s[i] < minS {
			start = edge{i, a[i], b[i], s[i], t[i]}
		}
	}
