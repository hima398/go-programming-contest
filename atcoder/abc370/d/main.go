...
func solve(h, w, q int, r, c []int) int {
	row := make([]*set.Set[int], h+2)
	for i := 0; i <= h+1; i++ {
		row[i] = set.New[int](comparator.IntComparator)
		for j := 0; j <= w+1; j++ {
			row[i].Insert(j)
		}
	}
	col := make([]*set.Set[int], w+2)
	for j := 0; j <= w+1; j++ {
		col[j] = set.New[int](comparator.IntComparator)
		for i := 0; i <= h+1; i++ {
			col[j].Insert(i)
		}
	}
	...
	for k := 0; k < q; k++ {
		if !row[r[k]].Contains(c[k]) {
			rowIt := row[r[k]].UpperBound(c[k])
			right := rowIt.Value()
			left := rowIt.Prev().Value()
			colIt := col[c[k]].UpperBound(r[k])
			bottom := colIt.Value()
			top := colIt.Prev().Value()
			...
			if bottom <= h {
				row[bottom].Erase(c[k])
				col[c[k]].Erase(bottom)
			}
			if top > 0 {
				row[top].Erase(c[k])
				col[c[k]].Erase(top)
