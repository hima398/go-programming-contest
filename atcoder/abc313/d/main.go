...
func main() {
	...
	printQuestion := func(k int, q []int) {
		defer out.Flush()
		fmt.Fprintf(out, "? %d", q[0])
		for i := 1; i < k; i++ {
			fmt.Fprintf(out, " %d", q[i])
		}
		fmt.Fprintln(out)
	}
	printAnswer := func(n int, ans []int) {
		defer out.Flush()
	...
	if k == 1 {
		...
		guessSequence := func(n, k int) []int {
			// k >= 2
			var r []int
			for i := 0; i < n; i++ {
				var q []int
			...
			for i := 0; i < n; i++ {
				next := (cur + k) % n
				c1[next] = c1[cur] ^ p[cur]
				c2[next] = c2[cur] ^ p[cur]
				cur = next
			}
			parity1 := 0
			for i := 0; i < k; i++ {
		...
		if Gcd(n, k) == 1 {
			printAnswer(n, guessSequence(n, k))
		} else {
			ans := guessSequence(n-1, k)
			//printAnswer(n-1, ans)
			var q []int
			for i := 0; i < k; i++ {
				q = append(q, n-i)
