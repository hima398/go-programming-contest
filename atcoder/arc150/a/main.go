...
func eval(n, k int, s string) bool {
	type node struct {
		s string
		l int
	}
	ns := []node{node{string(s[0]), 1}}
	for i := 1; i < n; i++ {
		if string(s[i]) == ns[len(ns)-1].s {
			ns[len(ns)-1].l++
		} else {
			ns = append(ns, node{string(s[i]), 1})
		}
	}
	idx := 0
	for {
...
func solve(t int, n, k []int, s []string) []string {
	var ans []string
	for i := 0; i < t; i++ {
		if eval(n[i], k[i], s[i]) {
			ans = append(ans, "Yes")
