...
func solve(n, m int, a, b []int, q int, x, y []int) []int {
	d := make([]int, n)
	e := make([][]int, n)
	for i := 0; i < m; i++ {
		d[a[i]]++
	...
	for i := 0; i < q; i++ {
		...
		if d[x[i]] >= t {
			fmt.Println("heavy")
			ans = append(ans, colors[x[i]])
			updatedAt[x[i]] = i
			colors[x[i]] = y[i]
		} else {
			fmt.Println("light")
			idx := updatedAt[x[i]]
			for _, next := range e[x[i]] {
				idx = Max(idx, updatedAt[next])
			}
			if idx < 0 {
				ans = append(ans, 1)
			} else {
				ans = append(ans, y[idx])
			}
			updatedAt[x[i]] = i
			for _, next := range e[x[i]] {
				updatedAt[next] = i
