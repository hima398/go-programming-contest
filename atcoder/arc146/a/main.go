...
func joinThreeCards(a, b, c int) int {
	res := 0
	s := []int{a, b, c}
	sort.Ints(s)
	for {
		sa, sb, sc := strconv.Itoa(s[0]), strconv.Itoa(s[1]), strconv.Itoa(s[2])
		candidate, _ := strconv.Atoi(sa + sb + sc)
...
func solveHonestly(n int, a []int) int {
	...
	for i := 0; i < n-2; i++ {
		for j := i + 1; j < n-1; j++ {
			for k := j + 1; k < n; k++ {
				ans = Max(ans, joinThreeCards(a[i], a[j], a[k]))
			}
		}
	}
	return ans
...
func solve(n int, a []int) int {
	sort.Ints(a)
	ans := joinThreeCards(a[n-1], a[n-2], a[n-3])
	return ans
}
