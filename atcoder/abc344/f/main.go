...
func main() {
	...
	var p [][]int
	for i := 0; i < n; i++ {
		p = append(p, nextIntSlice(n))
	}
	var r [][]int
	for i := 0; i < n; i++ {
		r = append(r, nextIntSlice(n-1))
	}
...
// 問題文のn, p, r, dが与えられたとき、kターン以内にp[n][n]に辿り着けるかを判定する
func check(n int, p, r, d [][]int, k int) bool {
	/*
		memo := make([][]int, n)
		for i := range memo {
			memo[i] = make([]int, n)
	...
	*/
	//
	//p[i][j]マスに、kターンで所持金lを持って辿り着けるか？
	var dfs func(i, j, k, l int) bool
	dfs = func(i, j, k, l int) bool {
		if i == 0 && j == 0 {
			return k >= 0 && p[i][j]*k >= l
		}
		ok := false
		if i > 1 {
			ok = ok || dfs(i-1, j, k-1, l+d[i-1][j])
		}
		if j > 1 {
			ok = ok || dfs(i, j-1, k-1, l+r[i][j-1])
		}
		return ok
		/*
			if memo[i][j] == 0 {
	...
	return dfs(n-1, n-1, k, 0)
}

func solve(n int, p, r, d [][]int) int {
	ng, ok := 0, int(1e18)
	for ok-ng > 1 {
		mid := (ok + ng) / 2
		if check(n, p, r, d, mid) {
			ok = mid
		} else {
			ng = mid
		}
