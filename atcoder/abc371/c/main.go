...
func main() {
	...
	ans := solve(n, mg, u, v, mh, a, b, la)
	Print(ans)
}

func solve(n, mg int, u, v []int, mh int, a, b []int, la [][]int) int {
	//8! = 40320
	...
	for {
		...
		for i := 0; i < n; i++ {
			for j := 0; j < n; j++ {
				if connected[i][j] != hConn[p[i]][p[j]] {
					cost += la[p[i]][p[j]]
					hConn[p[i]][p[j]] = connected[i][j]
					hConn[p[j]][p[i]] = connected[j][i]
				}
			}
		}
