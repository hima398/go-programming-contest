...
func main() {
	...
	ans := solve(n, x, p, q, l, r)
	for _, v := range ans {
		Print(v)
	}
}

func solve(n int, x, p []int, q int, l, r []int) []int {
	villages := set.New[int](comparator.IntComparator)
	mx := make(map[int]int)
	s := make(map[int]int)
	var sum int
	for i := 0; i < n; i++ {
	...
	const minL, maxR = -int(1e9) - 1, int(1e9 + 1)
	villages.Insert(minL)
	mx[minL] = 0
	s[minL] = 0
	villages.Insert(maxR)
	mx[maxR] = 0
	s[maxR] = sum

	var ans []int
	for i := 0; i < q; i++ {
		ll := villages.LowerBound(l[i]).Value()
		it := villages.LowerBound(r[i])
		var rr int
		if r[i] == it.Value() {
			rr = it.Value()
		} else {
