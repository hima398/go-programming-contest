...
func main() {
	...
	ans := solve(n, m, a, u, v, b)
	PrintHorizonaly(ans)
}

func solve(n, m int, a, u, v, b []int) []int {
	const INF = 1 << 60
	e := make([][]Edge, n)
	for i := 0; i < m; i++ {
		e[u[i]] = append(e[u[i]], Edge{v[i], b[i]})
		e[v[i]] = append(e[v[i]], Edge{u[i], b[i]})
	}
	...
	for q.Len() > 0 {
		...
		for _, next := range e[cur.t] {
			push(next.t, cur.w+next.w+a[next.t])
		}
	}
	return dist[1:]
}
