...
func solve(n, m int, a []int) int {
	...
	mm := make(map[int]int)
	//ms := set.NewMultiSet(comparator.IntComparator, set.WithGoroutineSafe())
	//ms := treemap.NewMultiMap[int, int](comparator.IntComparator, treemap.NewWithIntComparator())
	ms := set.New[int](comparator.IntComparator, set.WithGoroutineSafe())
	for _, ai := range a {
		mm[ai] += ai
		ms.Insert(ai)
	}
	...
	if m+ms.First().Value()-ms.Last().Value() == 1 {
		t := q[0]
		q = q[1:]
		q[len(q)-1] += t
	}
