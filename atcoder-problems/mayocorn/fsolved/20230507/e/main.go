...
func solve(n int, x, y, z []int) int {
	...
	computeCost := func(a, b, c, p, q, r int) int {
	...
	for _, pat := range ps {
		...
		for i := 0; i < n; i++ {
			...
			for ni := 0; ni < n; ni++ {
				...
				next := pat | (1 << ni)
				dp[next][ni] = Min(dp[next][ni], dp[pat][i]+computeCost(x[i], y[i], z[i], x[ni], y[ni], z[ni]))
			}
		}
	}
	s := (1 << n) - 1
	ans := INF
	for i := 1; i < n; i++ {
		ans = Min(ans, dp[s][i]+computeCost(x[i], y[i], z[i], x[0], y[0], z[0]))
	}
	return ans
}
