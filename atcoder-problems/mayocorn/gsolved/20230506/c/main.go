...
func solve(n, m int, u, v []int) bool {
	uf := NewUnionFind(n)
	for i := 0; i < m; i++ {
	...
	ok := true
	for i := 0; i < n; i++ {
		//連結成分のrootになっているものだけ調査する
		if i == uf.Find(i) {
			ok = ok && uf.Size(i) == uf.countPaths(i)
		}
	}
	return ok
}
...
func (uf *UnionFind) countPaths(x int) int {
	return uf.paths[uf.Find(x)]
}
...
func (uf *UnionFind) addPath(x int) {
	uf.paths[uf.Find(x)]++
}
