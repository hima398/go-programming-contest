...
func solve(n, m int, u, v []int) []int {
	const p = 998244353
	//(森の数)*k!/(m**k)
	comb := NewCombination(Max(n, m), p)
	im := Inv(m, p)
	var ans []int
	for k := 1; k <= n-1; k++ {
		numForest := comb.Combination(m, k)
		//TODO:閉路の数を引く
		prob := numForest
		prob *= comb.Factional(k)
		prob %= p
		prob *= Pow(im, k, p)
		prob %= p
		ans = append(ans, prob)
...
// m=n-1くらいしか解けない
func solve01(n, m int, u, v []int) []int {
	const p = 998244353
	uf := NewUnionFind(n)
	for i := 0; i < m; i++ {
	...
	var ans []int
	for k := 1; k <= n-1; k++ {
		prob := comb.Permutation(m, k) % p
		prob = prob * Pow(im, k, p) % p
		ans = append(ans, prob)
	}
	return ans
}
...
func (c *Comb) Permutation(n, k int) int {
	return c.fac[n] * c.finv[n-k] % c.p
}
